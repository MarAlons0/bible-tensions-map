...
An interactive web application for secular, academic analysis of the Bible as literature and historical document. It visualizes how theological and ethical tensions evolve across 83 biblical books — Old Testament, Apocrypha, and New Testament — through four complementary views.
...
- Starts with three aggregate rows — *Old Testament*, *Apocrypha*, and *New Testament* — each showing **median scores** across all their books.
...
**83 books**: 39 Old Testament + 17 Apocrypha + 27 New Testament.

The Apocryphal books follow the structure of the **New Oxford Annotated Bible (NRSV)**, organized into five sections: Narrative, Wisdom, Prophetic & Liturgical, Additions to Daniel, and Apocalyptic. These texts are canonical in Catholic and Orthodox traditions and are included here for their literary and historical significance.

**Scoring methodology**: Tension scores were generated by Claude (`claude-sonnet-4-6`, Anthropic), prompted to assess each book from a historical-critical standpoint informed by the scholarly tradition — including resources such as the New Oxford Annotated Bible and standard academic commentaries. They should be treated as an AI-assisted starting point for exploration, not as authoritative assessments. Reasonable scholars disagree on many of these readings.

**Biblical text**: Chapter-level analysis uses the public-domain **King James Version (KJV, 1611)**.
...
The database is seeded at startup via `python seed.py && gunicorn app:app`. The seed script is fully idempotent (upsert throughout), so it runs safely on every deployment restart. OT data comes from `seed_data.json`; Apocrypha from `seed_data_apocrypha.json`; NT data from `seed_data_nt.json`.

### Heatmap layering

All 83 books and their tension scores are loaded into the browser in a single API call (`/api/heatmap-full`). Expand/collapse state is managed entirely in JavaScript. Aggregate rows (testament and section) compute the **median** of non-null scores across their member books. No additional API calls are made when expanding or collapsing rows.
...
To regenerate the Apocrypha or NT seed data from the source scripts:
```bash
python generate_apocrypha_seed.py   # writes seed_data_apocrypha.json
python generate_nt_seed.py          # writes seed_data_nt.json
python seed.py                      # loads all three files into the database
```
